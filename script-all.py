...
SAVE_DIRECTORY = os.path.join(SCRIPT_DIR, "./emails")
SAVED_EMAILS_FILE = os.path.join(SCRIPT_DIR, "saved_emails.txt")
DELETE_EMAILS = False  

# Funktion zum Bereinigen des Betreffs
    ...
    for part, encoding in decoded_parts:
        try:
            if isinstance(part, bytes):
                ...
                if encoding.lower() == "unknown-8bit":  # Fix für unknown-8bit Fehler
                    encoding = "utf-8"
...
# Geladene Message-IDs aus Datei laden
...
# Funktion zum Laden der Konten aus accounts.txt
def load_accounts():
    accounts = []
